        ...
        <tbody>
          {clients.map((c) => (
            <tr key={c.id} className="border-b border-rule hover:bg-paper-raised">
              ...
              <td>{c.rescan_due_at ? new Date(c.rescan_due_at).toLocaleDateString("fr-FR") : <span className="text-ink-faint">non planifié</span>}</td>
              <td className="text-ink-faint">{new Date(c.created_at).toLocaleDateString("fr-FR")}</td>
            </tr>
          ))}
          {clients.length === 0 && (
            <tr><td colSpan={5} className="py-6 text-center text-ink-faint">Aucun client. Convertissez un lead !</td></tr>
          )}
        </tbody>
      </table>
    </div>
